...
impl ArrayBuffer {
    ...
    /// True when the bytes can change, or their pages can go away, while
    /// native code reads them. Another thread writes a `SharedArrayBuffer` at
    /// any point of the call. `resize()` unmaps the trimmed pages of a
    /// resizable buffer as soon as the call runs JS. A reader that reads a
    /// byte twice, sizes an output in one pass and fills it in another, or
    /// runs JS before it is done reads [`try_copy_bytes`](Self::try_copy_bytes).
